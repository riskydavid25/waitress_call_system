...
void resetSystem() {
  digitalWrite(greenLed, LOW);
  digitalWrite(blueLed, LOW);
  client.publish("waitress/sender2/call", "false");
  client.publish("waitress/sender2/bill", "false");
...
void loop() {
  ...
  if (callState == LOW && lastCallState == HIGH) {
    ...
    digitalWrite(blueLed, LOW);
    client.publish("waitress/sender2/call", "true");
    client.publish("waitress/sender2/bill", "false");
  ...
  if (billState == LOW && lastBillState == HIGH) {
    ...
    client.publish("waitress/sender2/call", "false");
    client.publish("waitress/sender2/bill", "true");
    Blynk.virtualWrite(V1, 0);
