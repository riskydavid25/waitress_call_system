...
void resetSystem() {
  digitalWrite(greenLed, LOW);
  digitalWrite(blueLed, LOW);
  client.publish("waitress/sender1/call", "false");
  client.publish("waitress/sender1/bill", "false");
...
void loop() {
  ...
  if (callState == LOW && lastCallState == HIGH) {
    ...
    digitalWrite(blueLed, LOW);
    client.publish("waitress/sender1/call", "true");
    client.publish("waitress/sender1/bill", "false");
  ...
  if (billState == LOW && lastBillState == HIGH) {
    ...
    client.publish("waitress/sender1/call", "false");
    client.publish("waitress/sender1/bill", "true");
    Blynk.virtualWrite(V1, 0);
